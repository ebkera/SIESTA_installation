...
# 1)First things first do this if you have not done them already
# date
# sudo apt update
...
tar xzf OpenBLAS.tar.gz && rm OpenBLAS.tar.gz
cd OpenBLAS-0.3.10
make TARGET=ZEN DYNAMIC_ARCH=0 CC=gcc FC=gfortran HOSTCC=gcc BINARY=64 INTERFACE=64 \
  NO_AFFINITY=1 NO_WARMUP=1 USE_OPENMP=0 USE_THREAD=0 USE_LOCKING=1 LIBNAMESUFFIX=nonthreaded
make TARGET=ZEN PREFIX=$OPENBLAS_DIR LIBNAMESUFFIX=nonthreaded install

#Below is the installation for blas version 0.3.10 and for the parallel installation For this is the one that is not recommended
# wget -O OpenBLAS.tar.gz https://github.com/xianyi/OpenBLAS/archive/v0.3.10.tar.gz
# tar xzf OpenBLAS.tar.gz && rm OpenBLAS.tar.gz
# cd OpenBLAS-0.3.10
# make DYNAMIC_ARCH=0 CC=gcc FC=gfortran HOSTCC=gcc BINARY=64 INTERFACE=64 \
...
# if proerly done you should get somehing like this  ### start
